...
def main():
    if len(sys.argv) < 2:
        ...
        print("  python -m catlm eval      Run personality eval cases")
        return
    ...
    if cmd == "prepare":
        ...
        import argparse
        parser = argparse.ArgumentParser(description="Chat with Cat")
        ...
        if args.prompt:
            print(f"Cat> {engine.chat([{'role': 'user', 'content': args.prompt}])}")
            return
